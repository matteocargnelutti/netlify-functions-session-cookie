...
//------------------------------------------------------------------------------
/**
 * Keeps a copy of environment variables before the tests run, 
 * so we can alter them on the fly safely.
 */
 ...
 * Before each test:
 * - Reset modules cache
 * - Reset `process.env` with its original state
 * - Pass fresh copy of `process.env` to the library via rewire's __set__, so it can be edited live.
 */
beforeEach(() => {
  jest.resetModules();
 ...
 * - Restore environment variables in their original state.
 */
afterAll(() => {
